...
Converted["_TextLabel"].Text = "W-MB | Hop rip_indra True Form "
...
Converted["_TextLabel1"].Text = "Status: Auto rip_indra True Form "
                    ...
                    function()
                        ...
                        if enemies:FindFirstChild("rip_indra True Form") then
                            for _, v in pairs(enemies:GetChildren()) do
                                if
                                    v.Name == "rip_indra True Form" and v:FindFirstChild("Humanoid") and
                            ...
                            local doughKing = game:GetService("ReplicatedStorage"):FindFirstChild("rip_indra True Form")
...
function GetServerWithRipIndra()
    local success, response = pcall(function()
        return HttpService:GetAsync("https://raw.githubusercontent.com/skibidip1/Hack/refs/heads/main/Api")
    end)
    
    if success then
        local jobId = response
        if jobId and jobId ~= "" then
            TeleportService:TeleportToPlaceInstance(game.PlaceId, jobId)
        else
            warn("Không có jobId hợp lệ từ API.")
        end
    else
        warn("Không thể kết nối tới API rip_indra.")
    end
end

-- Tự động gọi khi không có boss rip_indra
task.spawn(function()
